...
- 接管系统已有部署：检测到系统已装 Node.js + DSH 时，一键切换为直接管理（无需重装）
- 一键启动 / 停止 DSH（Web 服务，默认 127.0.0.1:3080）
- 监听端口设置、npm 镜像切换、启动后自动打开浏览器、开机自启
- 版本管理：Node.js / DSH 一键升级、降级、切换
- 多实例管理：配置多个 DSH 实例（名称/端口），随时切换
- 可选：把自管 `dsh` 命令加入用户 PATH，终端里直接可用
- 实时日志查看（按级别过滤、导出）
- 应用自身自动更新（GitHub Releases）
...
- [x] M2：版本管理（Node / DSH 升级降级）、开机自启、应用自动更新
